...
Stacked-PR tooling (such as GitHub's `gh stack`) conventionally opens each layer against the branch of the layer below rather than the final branch. bors understands that shape: a bare `bors stack` infers the parent from the base-branch chain, and when the bundle is queued bors retargets each layer's base onto the final branch itself (announcing it on the PR). Retargeting is not undone: if the bundle then fails or is canceled instead of merging, the bases stay on the final branch, and the recorded stack order still applies when it is re-approved. So landing a stack is just `bors stack` on each child plus the usual `bors r+` per layer.
