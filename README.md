...
When the commits must also appear in history in a fixed order, comment on the pull request that has to land second (`#456`):

    bors stack #123

This links `#456` into `#123`'s bundle and also guarantees that `#456` lands as a separate commit after `#123`, adjacent to it in history. For this to merge cleanly, the stacked PR's branch must be authored on top of the branch it stacks on. bors enforces this: the stack command is refused (and the bundle is held at approval time) whenever the stacked branch doesn't contain the current head of the PR it stacks on, e.g. after that PR was amended. Stacking cycles are rejected. The confirmation comment links a compare view of just the child's own changes, since the child PR's combined diff includes its parent's.
