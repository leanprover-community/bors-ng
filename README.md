...
When the changes must also apply in a fixed order, comment on the pull request that has to land second (`#456`):

    bors stack #123

This links `#456` into `#123`'s bundle and also guarantees that `#123`'s changes are applied first: with squash merges `#456` lands as its own commit directly after `#123`, and with merge commits the batch's single merge applies the two in that order. For this to merge cleanly, the stacked PR's branch must be authored on top of the branch it stacks on. bors enforces this: the stack command is refused (and the bundle is held at approval time) whenever the stacked branch doesn't contain the current head of the PR it stacks on, e.g. after that PR was amended. Stacking cycles are rejected. The confirmation comment links a compare view of just the child's own changes, since the child PR's combined diff includes its parent's.

Stacked-PR tooling conventionally opens each layer against the branch of the layer below rather than the final branch. bors understands that shape: a bare `bors stack` infers the parent from the base-branch chain, and when the bundle is queued bors retargets each layer's base onto the final branch itself (announcing it on the PR). If the bundle then fails or is canceled instead of merging, the bases stay on the final branch while the bundle exists, and the recorded stack order still applies when it is re-approved. `bors unlink` restores each base bors moved, unless it has been changed again by hand in the meantime. So landing a stack is just `bors stack` on each child plus the usual `bors r+` per layer.
