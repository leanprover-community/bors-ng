defmodule BorsNG.Worker.BatcherTest do
  ...
  test "rejects patch when CODEOWNERS parsing raises during CODEOWNERS check", %{proj: proj} do
    GitHub.ServerMock.put_state(%{
      {{:installation, 91}, 14} => %{
        branches: %{},
        commits: %{},
  ...
  test "waits when get_commit_reviews fails during preflight", %{proj: proj} do
    GitHub.ServerMock.put_state(%{
      {{:installation, 91}, 14} => %{
        branches: %{},
        commits: %{},
  ...
  test "marks batch as error when squash create_commit fails", %{proj: proj} do
    GitHub.ServerMock.put_state(%{
      {{:installation, 91}, 14} => %{
        branches: %{
          "master" => "ini",
